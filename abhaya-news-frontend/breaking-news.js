...
class BreakingNewsTicker {
    ...
    async fetchBreakingNews() {
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/news?breaking=true&limit=8&page=1&fields=_id,title,createdAt,category,breakingUrl`, {
            ...
            if (response.ok) {
                ...
                if (data.news && data.news.length > 0) {
                    return data.news.map(news => ({
                        ...
                        active: true,
                        category: news.category,
                        url: news.breakingUrl || '' // Use admin provided breaking URL if present
                    }));
                }
            }
        } catch (error) {
    ...
    openNewsArticle(newsId, newsUrl) {
        // If a custom URL was provided for breaking news, navigate there first
        if (newsUrl && newsUrl !== '') {
            window.location.href = newsUrl;
            return;
        }

        // Otherwise use the existing fetchNewsDetail function to open article in modal
        if (window.fetchNewsDetail && typeof window.fetchNewsDetail === 'function') {
            window.fetchNewsDetail(newsId);
        } else {
            // Fallback to backend news route
...
(function preloadBreakingNews() {
    ...
    const preloadPromise = fetch(`${API_BASE_URL}/news?breaking=true&limit=8&page=1&fields=_id,title,createdAt,category,breakingUrl`, {
        headers: {
            'Accept': 'application/json',
            'Cache-Control': 'no-cache'
        }
