...
uploadForm.addEventListener('submit', async (e) => {
    ...
    const breakingUrl = document.getElementById('breakingUrl').value || '';
    ...
    if (breakingUrl) formData.append('breakingUrl', breakingUrl);
        ...
        if (data.news && data.news.length > 0) {
            data.news.forEach(news => {
                ...
                row.innerHTML = `
                    ...
                    <td>${news.category}</td>
                    <td>${date}</td>
                    <td>${news.isBreaking ? '<strong style="color:#e11d48">Yes</strong>' : 'No'}</td>
                    <td class="action-buttons">
                        <button class="edit-btn" data-id="${news._id}">Edit</button>
                        <button class="delete-btn" data-id="${news._id}">Delete</button>
                        <button class="toggle-breaking-btn" data-id="${news._id}" data-breaking="${news.isBreaking}">${news.isBreaking ? 'Unmark Breaking' : 'Mark Breaking'}</button>
                    </td>
                `;
            ...
            document.querySelectorAll('.toggle-breaking-btn').forEach(btn => {
                btn.addEventListener('click', () => handleToggleBreaking(btn.dataset.id, btn.dataset.breaking === 'true'));
            });
        } else {
            const row = document.createElement('tr');
            row.innerHTML = '<td colspan="5">No news found</td>';
            newsTableBody.appendChild(row);
        }
    } catch (error) {
...
async function openEditModal(newsId) {
    ...
    try {
        ...
        document.getElementById('editIsBreaking').checked = !!news.isBreaking;
        document.getElementById('editBreakingUrl').value = news.breakingUrl || '';
...
async function handleEditNewsSubmit(e) {
    ...
    formData.append('category', category);
    formData.append('body', body);
    const editIsBreaking = document.getElementById('editIsBreaking').checked;
    const editBreakingUrl = document.getElementById('editBreakingUrl').value || '';
    formData.append('isBreaking', editIsBreaking);
    if (editBreakingUrl) formData.append('breakingUrl', editBreakingUrl);
...
// Handle toggling breaking status
async function handleToggleBreaking(newsId, currentlyBreaking) {
    const token = localStorage.getItem('token');
    if (!token) {
        window.location.href = 'index.html?status=needlogin';
        return;
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/news/${newsId}`, {
            ...
            headers: {
                'Authorization': `Bearer ${token}`,
                // 'Content-Type' intentionally omitted because we send FormData
            },
            body: (() => { const f = new FormData(); f.append('isBreaking', !currentlyBreaking); return f; })()
        });
        ...
        showSuccess(currentlyBreaking ? 'News unmarked as breaking' : 'News marked as breaking');
        loadNewsTable(currentPage);
    } catch (error) {
